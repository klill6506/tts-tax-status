...
**S-24 · [APP] · CANONICAL TAXPAYER IDENTITY — the `clients_tax_identity` unit (shape
Ken-ratified 2026-07-14 s83; SUITE_CONTRACT §3).** One row per taxpayer: SSN encrypted at
the column/application level, linked to `clients_client.id`, tax-app-only access, reads
audited. `returns_taxpayer.ssn` populates FROM it on return creation; in-return edits write
back; central record carries at most a derived `ssn_last4` for display. Enables what every
professional package keys on SSN: year-over-year matching, duplicate detection, proforma
keying. **HARD CONSTRAINTS: never an `ssn` column on the shared hub tables; match tokens
outside the encrypted table = keyed HMAC only, never a plain hash (pre-refused). Full SSN
never crosses the tax-app boundary — other suite apps match on email/phone/name
permanently (§3 ruling).** Sequence: after the S-23 security block (SEC-3 view-audit is a
natural prerequisite — identity reads should land in an audit trail that exists).
