...
## ▶ NOW WORKING ON — **S-19 usability batch 1, mid-stream — resume at item 6 (input widths)** (`USABILITY_QUEUE.md` in the tax-app root is the checklist; s38 shipped items 1/4/7/13/13b/15 + the CLIENT↔SERVER parity gate + the L3a spec-first unit RS `R008`; **s39 shipped 11 (focus-steal: stable row `_key` + dup-create guard) · 2 (entity-name-only header) · 3 (infinite scroll) · 5 (StateSelect state pass) + the dead "+ Add Shareholder/Partner/Officer" fix (mig 0181 prod-applied)**; remaining: 6 · 8+14 · G-a · 9 (RS spec first) · 12 (RS spec first) · 16; item 10 diagnosed — waiting on Ken's Lacerte reprint of file 1018). The 1120-S ATS completion lane stays Ken-gated: **e-services email OUT 2026-07-09** (S7/S8 ruling · 8941 key-inversion [Ken RATIFIED the finding — engine right, key wrong] · 1040 production flip · SOR re-request) → any S7/S8 builds → full-set bundle → the live upload loop. A2A (17g): IdenTrust packet OVERNIGHTED 2026-07-09, cert vetting ~7-14 days (⚠ 30-day download clock from notarization); AE values pre-agreed in `docs/mef/A2A_ENROLLMENT.md`. S-18: PWA CODE-COMPLETE (Ken install-verify pending) · 18f prefs DONE · 18a/18b/18g queued · 18d parked.
