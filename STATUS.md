...
- **`build_irs5695` is NOT blocked — SCOPED IN s174, READY TO BUILD, START HERE.** Only `NameLine1Txt` and `SSN` are required at top level (both already in the return header). The document is **two independent optional top-level groups** — `ResidentialCleanEnergyCrGrp` (§25D, lines 1–16) and `EgyEffcntHmImprvCrGrp` (§25C, lines 17a–32b) — so a §25D-only return emits just the first and is perfectly valid. Everything marked required in the XSD is required only *inside* a group that is itself optional, INCLUDING the item sub-groups (doors 19a–h, windows 20a–d, A/C 22a–d, water heater 23a–d, furnace 24a–d, panelboard 25b–e, heat pump 29a–h): **omit a sub-group entirely and its required children do not apply** — a §25C claim can emit 18a/18b + 27/28/30/31/32 with no item groups at all. ⚠ A flat "count the required elements" grep says 29 required and OVERSTATES it badly — check each element's NESTING DEPTH.
  - ⚠⚠ **THE DATA IS ON THE MODEL, NOT IN THE FFV ROWS.** The seeded `FORM_5695` rows are a **light face** with AGGREGATE keys (`l1_5`, `l18_26`, `l6b`, `l11`, `l13`–`l16`, `l19c`, `l19h`, `l1200`, `l29`–`l32`, `sch3_5a/5b`) — `l1_5` is lines 1–5b COMBINED, so `_form_scoped_lines` alone CANNOT produce per-line 1/2/3/4. The per-item granularity lives on **`Taxpayer.e5695_*`**: the costs (`_solar_electric`, `_solar_water`, `_small_wind`, `_geothermal`, `_battery`, `_fuel_cell_cost`, `_fuel_cell_kw`, `_insulation`, `_doors_top`, `_doors`, `_windows`, `_central_ac`, `_water_heater`, `_furnace`, `_panelboard`, `_home_audit`, `_heat_pump_biomass`), the tri-state gates (`_25c_main_home`, `_25c_original_user`, `_25c_five_years`, `_25c_construction`, `_fuel_cell_main_home`, `_battery_ge_3kwh`, `_energy_prop_*`, `_audit_qualified`, `_enabling_2025/_code`), the **QM ID PINs** (`_pin_doors`, `_pin_windows`, `_pin_central_ac`, `_pin_water_heater`, `_pin_furnace`, `_pin_panelboard`, `_pin_heat_pump_biomass`) and the home address. §25C(h) makes the QM IDs a CONDITION of the credit — emit them.
  - ⚠ **`render_5695` (renderer.py:11656, the v2 "whole face" version) ALREADY derives numeric-line values from exactly those model fields** and is verified. Do NOT re-derive that mapping blind — that is the s150/s173 8960 trap (a second copy that silently drifts). The XML's STRUCTURE differs (nested groups vs the renderer's flat line→(value,type) map) so it cannot simply consume the render face, but **which model field feeds which line is shared and must not fork**: follow the s173 precedent — read the same fields, and **PIN AGREEMENT WITH `render_5695` IN A TEST** rather than refactoring the working renderer. (The heavier alternative, if Ken wants it later, is the s159 `form7203_face.py` treatment: extract the face derivation and make the renderer delegate.)
  - ⚠ Tri-state gates are `None = not reached` — `render_5695`'s `put_yes_no` deliberately renders NEITHER box when unanswered, because a No forfeits the credit. The XML indicators must respect the same rule: **omit, never send a false**.
...
7. ✅ **DONE (s171, 2026-07-31) — Form 8615 line 1 per the 2025 i8615 "Line 1"
   instruction.** AGI for a no-earned-income child; the Child's Unearned Income
   Worksheet otherwise (line 9 − earned − the Sch 1/18 penalty; earned = 1z +
   Sch 1 3/6, losses at zero); the Alternate Worksheet cases (net SE loss / NOL)
   RED-defer via `ALT_L1_WORKSHEET` → NEW **D_8615_009**. Capital losses now
   reduce line 1. Both screens' copy rewritten; the s147 dollar proofs pin as
