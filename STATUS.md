...
##    (`D:\tax-test-data\tmp\batch-005\`: johnson-dennis · [client] ·
##    [client]). 7 still to author: JOHNSON DANIEL #2700 · KINCAID JOSHUA
...
##  - ⚠ [client] will no-tie on EIC (+255) until the eic_valid_ssn
...
- **1 refused by design**: [client] #1738 — client-copy print set, 1099-R
  detail pages absent (REVIEW_QUEUE; notes inventory what's ready).
- **[client] #1807 needed a data fix, then tied 17/17**: her shell carried a
...
- **3 committed HELD (all REVIEW_QUEUE, precise deltas):**
  - #2623 [client]: EIC −2,303 exactly — `eic_valid_ssn_taxpayer` (compute_eic
  ...
  - #2434 [client]: fed tax −118 on IDENTICAL taxable income (QDCGT-worksheet
...
- Post-commit diagnostics ran on ALL 19 committed clients both batches; only
  ...
  D_EIC_016 on [client], W-2 box-3/5-not-printed variance warnings, 2210/UET
...
- [client] JAMES L: NO hub client at all (REVIEW_QUEUE — not FINLEY
  ambiguity, the client simply isn't in the hub).

**Ken-blocked residuals (do NOT self-serve):** BARROW GA LIC ruling ·
FINLEY which-is-which · [client] missing client · [client] re-export ·
EIC filer-SSN allowlist build · [client] QDCGT question · HAWKS RIE question ·
...
1. **A non-virgin shell's overrides survive replace_documents** — replace
   ...
   OVERRIDES ([client] 8z) and prior EIC answers (CHANELL) persist and can
